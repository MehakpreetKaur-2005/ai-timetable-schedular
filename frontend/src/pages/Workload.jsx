...
import { HiOutlineCheck, HiOutlineXMark, HiOutlineUserGroup } from 'react-icons/hi2';
...
export default function Workload() {
    ...
    // Initialize assignments from context faculty when faculty changes
    if (faculty.length > 0 && !initialized) {
        const init = faculty.map(f => ({
            ...f,
            subjects: subjects.filter(s => s.departmentId === f.departmentId).map(s => s.id),
            unavailable: {},
        }));
        setAssignments(init);
        setSelectedFaculty(init[0]?.id || null);
        setInitialized(true);
    }
    ...
    const toggleSubject = (facultyId, subjectId) => {
        setAssignments(assignments.map(a => {
            ...
            const subs = a.subjects.includes(subjectId)
                ? a.subjects.filter(id => id !== subjectId)
                : [...a.subjects, subjectId];
            return { ...a, subjects: subs };
        }));
    };
            ...
            <div>
                ...
                <div className="card"><div className="card__body">
                    <div className="empty-state">
                        ...
                        <div className="empty-state__title">No faculty configured</div>
                        <div className="empty-state__description">Add faculty members first, then configure their workload here.</div>
                    </div>
                </div></div>
            </div>
        );
                        ...
                        <>
                            ...
                            <div className="card">
                                ...
                                <div className="card__body">
                                    {subjects.length === 0 ? (
                                        <p style={{ color: 'var(--gray-400)', fontSize: 'var(--font-sm)' }}>No subjects configured yet. Add subjects first.</p>
                                    ) : (
                            ...
                            <div className="card">
                                ...
                                <div className="card__body">
                                    {activeDays.length === 0 || periods.length === 0 ? (
                                        <p style={{ color: 'var(--gray-400)', fontSize: 'var(--font-sm)' }}>Configure working days and time slots first.</p>
                                    ) : (
