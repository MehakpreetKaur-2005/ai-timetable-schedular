...
export default function GenerateTimetable() {
    ...
    const [aiOptimizationRounds, setAiOptimizationRounds] = useState(3);
            ...
            {(state === 'idle' || state === 'error') && (
                <div className="card" style={{ marginBottom: 'var(--space-6)' }}>
                    ...
                    <div className="card__body">
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-6)', marginTop: 'var(--space-4)', paddingTop: 'var(--space-4)', borderTop: '1px solid var(--gray-100)' }}>
                            <div className="form-field">
                                <label className="form-field__label">AI Optimization</label>
                                <div className="toggle-field" style={{ marginTop: 8 }}>
                                    <div className={`toggle${useAiOptimization ? ' toggle--active' : ''}`} onClick={() => setUseAiOptimization(!useAiOptimization)}>
                                        <div className="toggle__knob"></div>
                                    </div>
                                    <span style={{ fontSize: 'var(--font-sm)', color: 'var(--gray-700)' }}>{useAiOptimization ? 'Enabled' : 'Disabled'}</span>
                                </div>
                                <span style={{ fontSize: 'var(--font-xs)', color: 'var(--gray-400)', marginTop: 4 }}>AI refines the GA output with targeted swaps</span>
                            </div>
                            {useAiOptimization && (
                                <div className="form-field">
                                    <label className="form-field__label">AI Optimization Rounds</label>
                                    <input className="form-field__input" type="number" min="1" max="10" value={aiOptimizationRounds} onChange={e => setAiOptimizationRounds(Number(e.target.value))} />
                                    <span style={{ fontSize: 'var(--font-xs)', color: 'var(--gray-400)', marginTop: 4 }}>1–10 refinement passes</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="generate-hero">
                        ...
                        {state === 'success' && result && (
                            <>
                                ...
                                {result.metadata?.ai_optimization?.enabled && (
                                    <div className="message-box message-box--info" style={{ maxWidth: 600, textAlign: 'left', marginBottom: 'var(--space-4)' }}>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-3)', margin: '12px 0 8px' }}>
                                            <div style={{ textAlign: 'center', padding: '8px', background: 'rgba(255,255,255,0.6)', borderRadius: 6 }}>
                                                <div style={{ fontWeight: 700, color: 'var(--primary-600)' }}>{result.metadata.ai_optimization.rounds_run}</div>
                                                <div style={{ fontSize: 'var(--font-xs)', color: 'var(--gray-500)' }}>Rounds</div>
                                            </div>
                                            <div style={{ textAlign: 'center', padding: '8px', background: 'rgba(255,255,255,0.6)', borderRadius: 6 }}>
                                                <div style={{ fontWeight: 700, color: 'var(--primary-600)' }}>{result.metadata.ai_optimization.total_swaps_applied}</div>
                                                <div style={{ fontSize: 'var(--font-xs)', color: 'var(--gray-500)' }}>Swaps Applied</div>
                                            </div>
                                            <div style={{ textAlign: 'center', padding: '8px', background: 'rgba(255,255,255,0.6)', borderRadius: 6 }}>
                                                <div style={{ fontWeight: 700, color: result.metadata.ai_optimization.fitness_after_ai > result.metadata.ai_optimization.fitness_before_ai ? 'var(--success-600)' : 'var(--gray-600)' }}>
                                                    {((result.metadata.ai_optimization.fitness_after_ai - result.metadata.ai_optimization.fitness_before_ai) * 100).toFixed(2)}%
                                                </div>
                                                <div style={{ fontSize: 'var(--font-xs)', color: 'var(--gray-500)' }}>Fitness Gain</div>
                                            </div>
                                        </div>
                                        <div style={{ fontSize: 'var(--font-xs)', color: 'var(--gray-500)' }}>
                                            GA: {(result.metadata.ai_optimization.fitness_before_ai * 100).toFixed(1)}%  →  AI: {(result.metadata.ai_optimization.fitness_after_ai * 100).toFixed(1)}%
                                        </div>
                                    </div>
                                )}
